...
export function quoteTransaction(
    ...
    let quote = 0
    if (tokenTo && tokenFrom && poolAmounts[tokenTo.uri] && poolAmounts[tokenFrom.uri]) {
        if (isAmountTo) {
            quote = (amount * poolAmounts[tokenFrom.uri]) / (poolAmounts[tokenTo.uri] - amount)
        } else {
            quote = poolAmounts[tokenTo.uri] * amount / (poolAmounts[tokenFrom.uri] + amount)
        }
    }
