...
export const exchangeTokens: TransactionFunction = (
  tokenFrom: Token,
  tokenTo: Token,
  amountIn: number,
  amountTo: number
  ...
  const res = (fcl.mutate as (args: any) => Promise<string>)({
    args: (arg: any, t: any) => [
    ...
    cadence: `
        ...
        transaction(amountIn: UFix64, minAmountOut: UFix64) {
          prepare(signer: AuthAccount) {
            ...
            let ${tokenFrom.name}Vault = signer.borrow<&${
      tokenFrom.name
    }.Vault>(from: ${tokenFrom.vaultPath}) 
              ?? panic("Could not borrow a reference to Vault")
        
            let token0Vault <- ${
              tokenFrom.name
            }Vault.withdraw(amount: amountIn) as! @${tokenFrom.name}.Vault
            let token1Vault <- ${pair.name}.${
      inOrder ? 'swapToken1ForToken2' : 'swapToken2ForToken1'
    }(from: <- token0Vault)
        
            if signer.borrow<&${tokenTo.name}.Vault>(from: ${
      tokenTo.vaultPath
    }) == nil {
                signer.save(<-${tokenTo.name}.createEmptyVault(), to: ${
      tokenTo.vaultPath
    })
            ...
            let ${tokenTo.name}TokenVault = signer.borrow<&${
      tokenTo.name
    }.Vault>(from: ${tokenTo.vaultPath}) 
              ?? panic("Could not borrow a reference to Vault")
    ...
    id,
    description: `Exchange ${round(amountIn, 4)} ${
      tokenFrom.ticker
    } for ${round(amountTo, 4)} ${tokenTo.ticker}`
  }));
