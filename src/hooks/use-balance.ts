...
export function useBalance(
  token: Token | SwapPair | string | undefined,
  addr: any
): number {
  ...
  if (!balances) {
    ...
    return (
      balances!.pairs.find((pair) => pair.pair.name == token.name)?.balance ?? 0
    );
  } else if (token instanceof Token) {
    return (
      balances!.tokens.find((t) => t.token.ticker == token.ticker)?.balance ?? 0
    );
  }
  return 0;
}
