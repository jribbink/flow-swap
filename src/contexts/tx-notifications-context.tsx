...
export const TXNotificationsProvider = ({
  children
}: {
  children: ReactNode;
}) => {
  ...
  useEffect(() => {
    const newTransactions = transactions.filter(
      (tx) => !lastTransactions.current.find((lastTx) => lastTx.id == tx.id)
    );
    const newNotifications = newTransactions.map<TXNotification>(
      (transaction) => {
        let issued = new Date();
        let expiry = new Date();
  ...
  useEffect(() => {
    (function progressUpdateLoop() {
      setNotifications((notifications) =>
        notifications.reduce((newNotifications, notification) => {
  ...
  const discardNotification = (notification: TXNotification) => {
    setNotifications(
      notifications.filter(
        (n) => n.transaction.id != notification.transaction.id
      )
    );
  };
