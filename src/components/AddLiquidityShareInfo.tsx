  ...
  return (
    <div className="d-flex flex-column">
      ...
      <div className="rounded-3 border bg-light d-flex flex-row mt-2">
        <PoolShareColumn
          valueText={round(priceBPerA, 8).toString()}
          labelText={labelBPerA}
        ></PoolShareColumn>
        <PoolShareColumn
          valueText={round(priceAPerB, 8).toString()}
          labelText={labelAPerB}
        ></PoolShareColumn>
        <PoolShareColumn
          valueText={numberToPercent(poolShare)}
          labelText={labelSharePercent}
        ></PoolShareColumn>
      </div>
    </div>
  );
...
const PoolShareColumn = ({
  ...
  return (
    <div className="d-flex flex-column p-2 text-center col">
      <span style={{ fontSize: '0.95em' }}>{valueText}</span>
      <span style={{ fontSize: '0.75em' }}>{labelText}</span>
    </div>
  );
};
