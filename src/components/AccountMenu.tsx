...
export default ({}: AccountMenuProps) => {
  ...
  const pendingTransactions = useContext(TransactionsContext)[0].filter(
    (tx) => tx.status == TransactionStatusCode.PENDING
  );
  ...
  return (
    <div className="d-flex bg-white rounded-4">
      ...
      <button
        className="btn btn-success shadow-none text-white rounded-4 p-2"
      ...
      >
        {pendingTransactions.length > 0 ? (
          ...
          <>
            <Breakpoint show="lg">{shortAddress(user.addr)}</Breakpoint>
            <Breakpoint hide="lg">{user.addr}</Breakpoint>
          </>
        )}
      </button>
      ...
      <AccountInfoModal
        ...
        onShowChange={setShowAccountInfo}
      ></AccountInfoModal>
    </div>
  );
};
...
function PendingTransactionsDisplay({ count }: { count: number }) {
  return (
    <div className="text-uppercase">
      ...
      <span className="ms-2">{count} Pending</span>
    </div>
  );
}
