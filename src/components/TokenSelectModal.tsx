...
export default ({
  ...
  return (
    <Modal
      ...
      dialogClassName="modal-sm modal-h-90 rounded-1"
    >
      <Modal.Header closeButton>
        <span style={{ fontSize: '1.2em' }}>Select a Token</span>
      </Modal.Header>
        ...
        <div className="list-group m-0">
          {tokens.map((token) => (
            <button
              ...
              style={
                !availableTokens.find((t) => t.ticker == token.ticker)
                  ? { opacity: '50%' }
                  : {}
              }
            ...
            >
              ...
              <span className="ps-2">{token.ticker}</span>
            </button>
          ))}
        </div>
      </Modal.Body>
