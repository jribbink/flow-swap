...
export default () => {
  ...
  useEffect(
    () => updateTokenAmounts(amountFrom, setAmountTo, false),
    [tokenTo, tokenFrom, amountFrom]
  );
  useEffect(
    () => updateTokenAmounts(amountTo, setAmountFrom, true),
    [amountTo]
  );
  ...
  return (
    <div
      className="bg-white mx-auto rounded-4 p-4 shadow"
    ...
    >
      ...
      <TransactionButton
        text="Swap"
        disabledText={getButtonDisabledText()}
        onClick={onSwapClick}
      ></TransactionButton>
    </div>
  );
